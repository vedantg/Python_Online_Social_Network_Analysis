...
test_tweets_file = 'test_tweet.txt'
file_name = 'Log.txt'
...
def write_response_file_func(write_response_file, listOfDicts):
    ...
    with open(test_data, 'a',encoding='utf-8') as fileWriter:
...
def get_max_id(tweets_list):
    """ 
    params:
    tweets_list : list of each tweet objects...
    """
    ids_list = []
...
def robust_request(twitter, resource, params):
    ...
    if response.status_code == 200:
        ...
        tweets_list = dataInJson['statuses']
        write_response_file_func(test_data, tweets_list)
...
def get_query_tweets(twitter, q):
    ...
    Args:
        ...
        q... A string to search tweets for
    """
    ###TODO
    ...
    while not loop_condition:
        ...
        total += len(tweets_list) 	# count tweets
        print("Tweets received: %s & total = %s" %(len(tweets_list), total) )
        if total >= 500:
            loop_condition = True
        write_response_file_func(test_data, tweets_list)
...
def create_user_tweet_data(fname,tweets_data_file):
    ...
    fp_t = open(tweets_data_file, 'a',encoding='utf-8')
    ...
    with open(fname, 'r',encoding='utf-8') as fp:
        for line in fp:
            ...
            for tweets in tweets_list:
                      ...
                      fp_t.write(tw['id_str']+ " || "+tw['screen_name']+ " || "+ tweet_text+ "\n")
    fp_t.close() 
...
def read_tweets(fname):
    ...
    with open(fname, 'r',encoding='utf-8') as fp:
        for line in fp:
            temp_tweets.append(line.split(" || ")[2])
            #print("TWEETS::",temp_tweets)
    
    tweets_list = list(set(temp_tweets))
...
def read_data(path):
    ...
    #print("Typeof Fname::",type(fnames))
    ...
    #print("Typeof Fname_NEG::",type(fnames))
    #print("FNAMES_NEG::",len(fnames))
    data += [(0, open(f,encoding='utf-8').readlines()[0]) for f in sorted(fnames)]
    #print("Typeof data1::",type(data))
    #print("data1::",len(data))
    data = sorted(data, key=lambda x: x[1])
    #print("Typeof data2::",type(data))
...
def read_test_data(path):
    ...
    for f in sorted(fnames):
        print("F::",f)
        line = open(f,encoding='utf-8').readlines()
...
def parse_test_data(best_result, vocab):
    ...
    docs,labels=read_test_data(os.path.join('data', 'test'))
...
def print_top_misclassified(test_docs, test_labels, X_test, clf):
    """
    ...
    dictlist_return = sorted(dictlist_return, key=lambda x: x['probas'][x['truth']])
...
def main():
    ...
    create_user_tweet_data(test_data,test_tweets_file)
    
    tweets_list = read_tweets('test_tweet.txt')
    print("total Tweets: %s" %len(tweets_list))
    file_name_ = 'test_tweet_'
    suffix = '.txt'
    i = 0
    filepath = os.path.abspath("data/test/pos")
    ...
    for t in tweets_list:
        filepath_1 = os.path.join(filepath, file_name_ + str(i) + suffix)
        f = open(filepath_1, "w",encoding='utf-8')
        f.write(t)
    ...
    number_class = print_top_misclassified(test_docs, test_labels, X_test, clf)
    print("number_class",number_class)
    write_result(file_name, number_class)
